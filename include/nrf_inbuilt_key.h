...
/**
 * @brief Initialize inbuilt key management module.
 *
 * @note In case of failure, all calls to functions in \ref nrf_inbuilt_key
 *       module will fail with NRF_EIO error code.
 */
uint32_t nrf_inbuilt_key_init(void);


/**
 * @brief De-initialize inbuilt key management module.
 */
 ...
 * The credential can later be referenced for use or managed by nrf_inbuilt_key
 * module by using the application defined sec_tag.
 *
 * @note Run this function only when LTE link is inactive. If used during active
 *       link, NRF_EACCES will be returned and the key material will not be
 * written.
 *
 * @param[in]  sec_tag        Application defined tag for this credential to be
 * referred to in setting up a BSD Secure Socket or to manage the credential
 *                            using nrf_key_mgmt module to read/delete/search
 * the key.
 * @param[in]  cred_type      Type of credential being created and stored for
 * later use.
 ...
 * @retval NRF_EIO      If operation was not successful due to internal error or
 * uninitialized module.
 * @retval NRF_ENOBUFS  If the operation could not be performed because it could
 * not allocate enough intermediate buffers to perform the operation.
 * @retval NRF_ENOENT   If the sec_tag indicated cannot be written.
 * @retval NRF_ENOMEM   If there was not memory enough to store the credential
 * data.
 * @retval NRF_EPERM    If the application did not have permission to do the
 * operation.
 * @retval NRF_EACCES   If the operation could not be performed while modem is
 * in active state.
 ...
 *
 * This function will read the credential from persistent memory referenced by a
 * sec_tag.
 *
 * @param[in]     sec_tag       Application defined tag for this credential to
 * read.
 * @param[in]     cred_type     Type of credential being read.
 * @param[in]     p_buffer      Output buffer containing where to write the read
 * credential data.
 * @param[inout]  p_buffer_len  Length of the output buffer as input, and length
 ...
 * @retval NRF_ENOBUFS  If the operation could not be performed because it could
 * not allocate enough intermediate buffers to perform the operation.
 * @retval NRF_ENOENT   If there was no credential associated with the sec_tag
 * and cred_type.
 * @retval NRF_EPERM    If the application did not have permission to do the
 * operation.
 * @retval NRF_EINVAL   If provided buffer is to small for result data. If
 * failing with this error, the size needed is provided as output parameter by
 * reference in the output p_buffer_len output parameter.
 */
 ...
 * @retval NRF_EIO      If operation was not successful due to internal errors
 ...
 *
 * @param[in]  sec_tag        Application defined tag for this credential apply
 * the permissions to.
